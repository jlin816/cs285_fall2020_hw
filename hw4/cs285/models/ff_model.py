...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        # the output of the network is the
        # *normalized change* in state, i.e. normalized(s_t+1 - s_t).
        delta_pred_normalized = self.delta_network(concatenated_input)
        # unnormalize the output to get the delta
        next_obs_pred = obs_unnormalized + (delta_pred_normalized * self.delta_std + self.delta_mean)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        pred_delta_normalized = self.forward(observations, actions, **data_statistics)[1]
        loss = self.loss(target, pred_delta_normalized)
